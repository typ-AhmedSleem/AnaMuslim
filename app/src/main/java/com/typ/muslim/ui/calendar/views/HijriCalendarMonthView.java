...
package com.typ.muslim.ui.calendar.views;
...
public class HijriCalendarMonthView extends MonthView {
	...
	public HijriCalendarMonthView(Context context) {
		...
		textSize = DisplayUtils.sp2px(context, 14f);
	...
	protected void onDrawText(Canvas canvas, Calendar cal, int x, int y, boolean hasScheme, boolean isSelected) {
		...
		dayTextPaint.setTextSize(textSize);
		y = (int) (y + (mItemHeight / 2f) + (dayTextPaint.measureText(String.valueOf(cal.getDay())) / 2f) - padding);
		canvas.drawText(
				String.valueOf(cal.getDay()),
				x + mItemWidth / 2f,
				y,
