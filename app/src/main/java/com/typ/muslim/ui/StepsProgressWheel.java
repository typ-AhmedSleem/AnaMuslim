...
public final class StepsProgressWheel extends BaseView {
	...
	private @Nullable String textInfo;
	private @Nullable String textValue;
	private @ColorInt int valueTextColor;
	...
	public StepsProgressWheel(Context context, @Nullable AttributeSet attrs) {
		...
		Consumers.doIf(() -> steps = EasyList.createList(
				new ProgressStep(true, R.color.red),
				new ProgressStep(false, R.color.green),
				new ProgressStep(true, R.color.yellow),
				new ProgressStep(false, R.color.orange),
				new ProgressStep(true, R.color.blue)), isInEditMode());
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		steps.iterate((pos, step) -> {
			if (step.isDone()) {
				...
				// Paint color
				paintStep.setColor(getColor(step.getColor()));
				// Draw the arc
				canvas.drawArc(
						rectBar,
						angleStep * (pos),
