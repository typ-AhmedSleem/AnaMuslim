...
public class QuickSettingsFragment extends Fragment {
    ...
    private int[] notifyBeforePray = new int[5];
    private boolean[] iqamaAfterPrays = new boolean[5];
    private int[] muteDuringPrays = new int[5];
...
//                Toast.makeText(requireContext(), "Back is pressed", Toast.LENGTH_SHORT).show();
                ...
                new ViewManager.PrayMinutesPickerBottomSheet<Integer[]>(requireContext(), R.string.notify_before_pray, R.string.notify_before_pray, AMSettings.getNotifyTimeBeforePrays(requireContext()), new int[]{-60, 60}) {
                    ...
                    public void onResult(Integer[] results) {
                        for (Prays whatPray : Prays.values()) {
                            ...
                            notifyBeforePray[whatPray.ordinalWithoutSunrise()] = results[whatPray.ordinalWithoutSunrise()];
                        }
                        // Refresh Views
                        refreshViews();
                    }
                ...
                synchronized (this) {
                    new ViewManager.PrayMinutesPickerBottomSheet<Integer[]>(requireContext(), R.string.mute_during_pray, R.string.mute_during_pray, AMSettings.getMuteTimeDuringPrays(requireContext()), new int[]{-60, 60}) {
                        ...
                        public void onResult(Integer[] results) {
                            for (Prays whatPray : Prays.values()) {
                                if (whatPray == Prays.SUNRISE) continue;
                                muteDuringPrays[whatPray.ordinalWithoutSunrise()] = results[whatPray.ordinalWithoutSunrise()];
                            }
                        }
                    };
                    // Refresh views
        ...
        fragmentView.findViewById(R.id.btn_finish).setOnTouchListener(new EnhancedScaleTouchListener(100, 0.97f, 0.9f) {
            ...
            private void saveSettings() {
                ...
                for (Prays pray : Prays.values()) {
                    ...
                    AMSettings.save(requireContext(), Keys.NOTIFY_BEFORE_PRAY(pray), notifyBeforePray[pray.ordinalWithoutSunrise()]);
                    AMSettings.save(requireContext(), Keys.IQAMA_ENABLED_FOR(pray), iqamaAfterPrays[pray.ordinalWithoutSunrise()]);
                    AMSettings.save(requireContext(), Keys.MUTE_DURING_PRAY(pray), muteDuringPrays[pray.ordinalWithoutSunrise()]);
                }
                ...
                AMSettings.save(requireContext(), Keys.IS_FIRST_RUN, false);
            }
        });
    }
    ...
    private void changeTextViewStatus(@NotNull MaterialTextView tv, @StringRes int text, int status) {
        tv.setTextColor(ResMan.getColor(requireContext(), status == -1 ? R.color.red : status == 0 ? R.color.ef_colorPrimaryDark : R.color.green));
