...
public class AMBottomBar extends MaterialCardView {
	...
	private void prepareBottomBar(Context context) {
		...
		super.setCardBackgroundColor(AMRes.getColor(context, R.color.transparent));
	...
	private void selectTab(int selectedTab) {
		...
		for (int index = 0; index < tabsIVs.length; index++) {
			if (this.currentTab == index) tabsIVs[this.currentTab].setColorFilter(AMRes.getColor(getContext(), R.color.bottomBarSelectedTab)); // Show selection on current tab.
			else tabsIVs[index].setColorFilter(AMRes.getColor(getContext(), R.color.bottomBarUnSelectedTab)); // Clear selection from other tabs.
		}
	}
