...
public class SelectLocationActivity extends AppCompatActivity {
    ...
    private static final class CitiesAdapter extends RecyclerArrayAdapter<Location> {
        ...
        public void refreshLocationsWithQuery(@Nullable String query) {
            ...
            // Refresh data of RV
            final List<Location> locs = TextUtils.isEmpty(query) ? this.locations : queryLocations(query);
            this.removeAll(); // Remove all locations.
            this.addAll(locs); // Add locations.
        }
        ...
        private List<Location> queryLocations(String query) {
            ...
            for (Location loc : this.locations) {
                if (loc != null && (loc.getCityName().toLowerCase(Locale.ROOT).trim().contains(query) || loc.getCountryName().toLowerCase(Locale.ROOT).trim().contains(query))) {
