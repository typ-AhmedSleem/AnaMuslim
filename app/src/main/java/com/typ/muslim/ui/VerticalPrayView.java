...
public class VerticalPrayView extends DashboardCard {
	...
	public void parseAttrs(Context context, AttributeSet attrs) {
		...
		TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.PrayItemView);
		Prays pray = Prays.valueOf(typedArray.getInt(R.styleable.PrayItemView_pivPray, FAJR.ordinal()));
		this.pray = new Pray(pray, pray.name(), 0);
	...
	public void prepareCardView(Context context) {
		...
		ibtnChangeNotifyMethod.setOnClickListener(this::onClick);
		// Refresh UI
		this.setPray(pray);
	}
	...
	public VerticalPrayView setPray(Pray pray) {
		// Null check
		if (pray == null || isInEditMode()) return this;
		// Update runtime
		this.pray = pray;
		final Pray nextPray = PrayerManager.getNextPray(getContext());
		// PrayName
		this.tvPrayName.reset();
		this.tvPrayName.addSlice(new Slice.Builder(getString(pray.getPrayNameRes()))
				.style(Typeface.BOLD)
				.textSize(sp2px(16f))
				.textColor(getColor(isPrayPassed() ? R.color.green : nextPray.equals(pray) ? pray.getType().getOnSurfaceColorRes() : R.color.darkAdaptiveColor))
				.build());
		if (RamadanManager.isInRamadan() && (pray.getType() == FAJR || pray.getType() == MAGHRIB || pray.getType() == ISHA)) {
			...
			else if (pray.getType() == ISHA) sliceText = getString(R.string.taraweeh);
			else sliceText = "";
		...
		this.tvPrayTime.setTextColor(getColor(isPrayPassed() ? R.color.green : nextPray.equals(pray) ? pray.getType().getOnSurfaceColorRes() : R.color.darkAdaptiveColor));
	...
	public void changeIndicator(Pray nextPray) {
		if (this.isPrayPassed()) {
			...
			this.animateCardColor(getColor(R.color.adaptiveBackgroundColor));
			ivIndicator.setBackgroundTintList(ColorStateList.valueOf(getColor(R.color.green)));
		} else if (pray.equals(nextPray)) {
			// Next
			final int surfaceColor = getColor(pray.getType().getSurfaceColorRes());
			final int onSurfaceColor = getColor(pray.getType().getOnSurfaceColorRes());
			this.animateCardColor(surfaceColor);
			tvPrayName.setTextColor(onSurfaceColor);
			tvPrayTime.setTextColor(onSurfaceColor);
			ivIndicator.setColorFilter(onSurfaceColor);
			ivIndicator.setImageResource(R.drawable.ic_arrow_to_right);
			ivIndicator.setBackgroundResource(R.drawable.shape_next_pray);
			ivIndicator.setBackgroundTintList(ColorStateList.valueOf(onSurfaceColor));
		} else {
			// Coming
