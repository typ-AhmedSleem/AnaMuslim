...
//        this.qiblaAngle = 136.04663f;
        ...
        this.sensorManager.registerListener(this, magneticSensor, SensorManager.SENSOR_DELAY_NORMAL, SensorManager.SENSOR_DELAY_UI);
        this.sensorManager.registerListener(this, accelerometerSensor, SensorManager.SENSOR_DELAY_NORMAL, SensorManager.SENSOR_DELAY_UI);
...
//            AManager.log(TAG, "current mode [%s] isNewMode2d[%s] GY_OLD[%f] GY_NEW[%f]", currMode, isNewMode2D, currGravityY, gravityMatrix[1]);
        ...
        // Vibrate sensor (if vibration is enabled)
        if (this.isArrowBearingToZero(targetQiblaAngle) && isVibrationEnabled) {
            if (!isVibrating) {
