...
import com.typ.muslim.activities.names.HolyNamesOfAllahActivity;
...
public class BottomSheets {
	...
	public static abstract class BaseBottomSheet extends BottomSheetBehavior.BottomSheetCallback {
		...
		public final int getColor(@ColorRes int colorRes) {
			return AMRes.getColor(context, colorRes);
		}

		@NonNull
		public final String getString(@StringRes int stringRes) {
			return AMRes.getString(context, stringRes);
		}
	...
	public static class TodayPraysBottomSheet extends BaseBottomSheet {
		...
		// Views
		private SpannableTextView stvCurrPray, stvNextPray, stvTodayHijri;
		...
		public void prepareInnerViews() {
			...
			this.stvTodayHijri = $(R.id.stv_hijri_date);
		...
		public void bindInnerViews() {
			...
			stvNextPray.addSlice(new Slice.Builder(String.format(Locale.getDefault(), "%s\n", getString(R.string.next_pray)))
					...
					.build());
			stvNextPray.addSlice(new Slice.Builder(getString(nextPray.getPrayNameRes()))
			...
			if (currPray != null) {
				...
				// Current Pray is Isha and current day has finished
				currPrayName = getString(R.string.isha);
			}
			...
			stvCurrPray.addSlice(new Slice.Builder(currPrayName)
					...
					.build());
			// Hijri date
			final HijriDate todayHijri = HijriCalendar.getToday();
			stvTodayHijri.addSlice(new Slice.Builder(todayHijri.getDay() + "\n")
					.style(Typeface.BOLD)
			        .textSize(sp2px(context, 30f))
			        .textColor(getColor(R.color.colorPrimary))
					.build());
			stvTodayHijri.addSlice(new Slice.Builder(String.format(Locale.getDefault(),"%s,%d%s", todayHijri.getMonthName(), todayHijri.getYear(), getString(R.string.H)))
	...
	public static class TasbeehOptionsBottomSheet extends BaseBottomSheet {
		...
		public void bindInnerViews() {
			...
			TextDrawable targetIcon = TextDrawable.builder().beginConfig()
			                                      ...
			                                      .width(90)
			                                      .height(90)
			                                      .textColor(AMRes.getColor(context, R.color.darkAdaptiveColor))
			                                      .endConfig()
		...
		public void setupListeners() {
			...
			this.btnChangeTarget.setOnClickListener(v -> {
				...
				if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) selectedItem.setIconTintList(ColorStateList.valueOf(AMRes.getColor(context, R.color.green)));
				// Show the popup
				popupMenu.show();
			});
	...
	public static class AnswerPrayedBottomSheet extends BaseBottomSheet {
		...
		@Override
		public void prepareInnerViews() {
			this.tvPrayName = $(R.id.tv_pray_name);
			this.tvPrayTime = $(R.id.tv_pray_time);
		...
		public void bindInnerViews() {
			...
			this.tvPrayName.addSlice(new Slice(
					new Slice.Builder(String.format(Locale.getDefault(), "%s, ", AMRes.getString(context, R.string.have_you_prayed)))
							.textSize(45)
							.textColor(AMRes.getColor(context, R.color.darkAdaptiveColor))));
			this.tvPrayName.addSlice(new Slice(
					new Slice.Builder(AMRes.getString(context, whichPray.getPrayNameRes()))
							.textSize(65)
							.style(Typeface.BOLD)
							.textColor(AMRes.getColor(context, R.color.green))));
			this.tvPrayName.display();
			// PrayTime
			this.tvPrayTime.setText(whichPray.getFormattedTime(context));
		}
